...
export default function Login({ onLogin, setView, setUserRole }) {
  ...
  return (
    <div className="max-w-md mx-auto my-12 bg-white border border-lua-rose-dark/10 p-8 rounded-2xl shadow-xs transition-all">
      <div className="text-center mb-8">
        <span className="text-4xl text-lua-gold block mb-2">🌙</span>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {isSignUp && (
          <>
            <div>
              ...
              <input type="text" placeholder="Seu nome" value={nome} onChange={(e) => setNome(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-lua-rose-dark transition-colors" required />
            </div>
            
            <div>
              <label className="text-xs font-semibold uppercase tracking-wider text-slate-500 block mb-1">WhatsApp</label>
              <input type="text" placeholder="(85) 99999-0000" value={whatsapp} onChange={handleWhatsappChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-lua-rose-dark transition-colors" required />
            </div>

            <div className="grid grid-cols-3 gap-4">
              <div>
                <label className="text-xs font-semibold uppercase tracking-wider text-slate-500 block mb-1">UF</label>
                <select value={uf} onChange={(e) => setUf(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-lua-rose-dark transition-colors" required>
              ...
              <div className="col-span-2">
                ...
                <select value={cidade} onChange={(e) => setCidade(e.target.value)} disabled={carregandoCidades} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-lua-rose-dark transition-colors disabled:opacity-50" required>
        ...
        <div>
          ...
          <input type="email" placeholder="seuemail@provedor.com" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-lua-rose-dark transition-colors" required />
        </div>

        <div>
          <label className="text-xs font-semibold uppercase tracking-wider text-slate-500 block mb-1">Senha</label>
          <input type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-lua-rose-dark transition-colors" required />
        </div>

        <Button variant="primary" type="submit" className="w-full mt-2" disabled={carregando}>
          {carregando ? 'Processando...' : isSignUp ? 'Concluir Cadastro' : 'Entrar no Sistema'}
        </Button>
      </form>

      <div className="mt-6 text-center border-t border-slate-100 pt-4">
        <button type="button" onClick={() => setIsSignUp(!isSignUp)} className="text-xs text-lua-rose-dark hover:underline font-medium">
